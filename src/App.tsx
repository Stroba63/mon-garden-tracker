    ...
    <div className="min-h-screen bg-[#060a0e] text-white font-sans p-4 flex flex-col items-center pb-20">
      ...
      <div className="w-full max-w-md bg-[#111821] border-x border-slate-800/50">
        <div className="grid grid-cols-2 divide-x divide-slate-800/50">
          <div className="p-8 text-center border-b border-slate-800/50">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Œufs Ouverts</p>
            <p className="text-4xl font-black text-white tabular-nums tracking-tighter">{totalEggs}</p>
          </div>
          <div className="p-8 text-center bg-orange-500/[0.03] border-b border-slate-800/50">
            <p className="text-[10px] font-black text-orange-500/80 uppercase tracking-widest mb-1 italic">Pity Actuelle</p>
            <p className="text-4xl font-black text-orange-400 tabular-nums tracking-tighter">{currentStreak}</p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full max-w-md mt-12 space-y-6">
        <div className="flex justify-between items-center px-6">
          ...
          {history.length > 0 && (
            <button onClick={() => setIsResetModalOpen(true)} className="text-slate-800 hover:text-red-500 p-2 transition-colors" title="Réinitialiser les statistiques">
                ...
                <div key={h.id} className="bg-[#111821] border border-slate-800/40 rounded-3xl p-5 flex items-center gap-4 shadow-lg hover:border-slate-700 transition-colors">
                  ...
                  <div className="flex-1 flex flex-col justify-center gap-3">
                    <div className="flex justify-between items-start">
                      <span className="text-orange-500 font-black uppercase tracking-wider text-base drop-shadow-sm leading-none pt-1">{petInfo.name}</span>
                      <span className="text-[10px] text-slate-500 font-bold tabular-nums text-right leading-tight">{h.date}</span>
                    </div>
                    <div className="flex justify-between items-end">
                      <div className="bg-emerald-500/10 text-emerald-400 px-4 py-2 rounded-xl border border-emerald-500/30 font-black flex flex-col justify-center shadow-sm min-w-[70px]">
                        <span className="text-[9px] opacity-70 uppercase tracking-widest mb-1">Poids</span>
                        <span className="text-base leading-none">{h.weight} <span className="text-xs opacity-80">KG</span></span>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest leading-none mb-1.5">Chance</p>
                        <p className="text-3xl font-black text-white tabular-nums leading-none tracking-tighter">{h.eggsTaken}</p>
                      </div>
                    </div>
                  </div>
                </div>
