...
const RAW_PETS = [
  ...
  'giantswan',
  'giftrat',
  'gildedchocchocolatebunny',
  'gildedchoceasterbunny',
  'gildedchoceastereggchick',
  'gildedchocjerboa',
  'gildedchocmarshmallowlamb',
  'gildedchocnyala',
  'gildedchocperyton',
  'gildedchocspringbee',
  'giraffe',
  ...
  'rainbowankylosaurus',
  'rainbowarcticfox',
  'rainbowbeardeddragon',
  'rainbowbearonbike',
  'rainbowbirb',
  'rainbowblackbird',
  'rainbowbrownowl',
  'rainbowcarnivalelephant',
  'rainbowcelebrationpuppy',
  'rainbowcerberus',
  'rainbowchinchilla',
  'rainbowchristmasgorilla',
  'rainbowckitsune',
  'rainbowclam',
  'rainbowcuckoo',
  'rainbowdilo',
  ...
  'rainbowmizuchi',
  'rainbownewyearsbird',
  'rainbownewyearschimp',
  'rainbownewyearsdragon',
  'rainbowoxpecker',
  'rainbowpachycephalo',
  'rainbowparasaurolophus',
  'rainbowperformerseal',
  'rainbowphoenix',
  'rainbowpinkbunny',
  'rainbowrhino',
  'rainbowshowpony',
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        // Si aucun utilisateur n'est connecté, on connecte en fantôme par défaut
        try {
          if (
  ...
  const handleLogout = async () => {
    try {
      await signOut(auth);
      // Après déconnexion, le système reconnectera automatiquement en Anonyme grâce au useEffect
      setTotalEggs(0);
      setCurrentStreak(0);
      setHistory([]);
    } catch (error) {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snap) => {
        if (snap.exists()) {
          ...
          // Optionnel : remettre à zéro si le doc n'existe pas
          setTotalEggs(0);
          setCurrentStreak(0);
          setHistory([]);
        }
  ...
  return (
    <div className="min-h-screen bg-[#060a0e] text-white font-sans p-4 flex flex-col items-center pb-20">
      ...
      <div className="w-full max-w-md flex justify-between items-center mb-6 px-4">
        {/* Bouton de connexion au lieu de "Serveur Privé" */}
        {user && !user.isAnonymous ? (
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 bg-emerald-900/40 hover:bg-emerald-800/60 transition-colors px-3 py-1.5 rounded-full border border-emerald-700 shadow-[0_0_10px_rgba(16,185,129,0.2)]"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 bg-blue-900/40 hover:bg-blue-800/60 transition-colors px-3 py-1.5 rounded-full border border-blue-700 shadow-[0_0_10px_rgba(59,130,246,0.2)] group"
          >
        ...
        <div className="flex items-center gap-2">
          {isSaving ? (
            <span className="text-[9px] font-black text-emerald-400 animate-pulse uppercase">
              Sauvegarde...
            </span>
          ) : (
      ...
      {/* Header (Style Grow a Garden) */}
      <div className="w-full max-w-md bg-gradient-to-b from-[#107c64] to-[#0a4d3e] p-8 rounded-t-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-row items-center justify-center gap-6 relative border-t border-white/10 overflow-hidden">
        {/* L'OEUF avec effet de lumière */}
        ...
        <h1 className="text-2xl font-black text-white uppercase tracking-wider text-left drop-shadow-lg leading-tight">
          GROW A GARDEN
          <br />
          <span className="text-emerald-300 text-base opacity-80">
            Tracker Personnel
          </span>
        </h1>
      </div>
      ...
      {/* Zone Statistiques */}
      <div className="w-full max-w-md bg-[#111821] border-x border-slate-800/50">
        <div className="grid grid-cols-2 divide-x divide-slate-800/50">
          ...
          <div className="p-8 text-center bg-orange-500/[0.03] border-b border-slate-800/50">
            <p className="text-[10px] font-black text-orange-500/80 uppercase tracking-widest mb-1 italic">
              Pity Actuelle
            </p>
            <p className="text-4xl font-black text-orange-400 tabular-nums tracking-tighter">
              {currentStreak}
            </p>
          </div>
        </div>
      ...
      <div className="w-full max-w-md mt-12 space-y-6">
        <div className="flex justify-between items-center px-6">
          <h2 className="text-xs font-black text-slate-600 uppercase tracking-[0.25em] flex items-center gap-2">
            <History className="w-4 h-4" /> Journal des Huges
          </h2>
          {history.length > 0 && (
            <button
        ...
        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar px-2">
          {history.length === 0 ? (
            <div className="text-center py-16 bg-[#0a0f14] rounded-[2.5rem] border border-dashed border-slate-800/50 text-slate-800 text-[10px] font-black uppercase tracking-widest flex flex-col items-center gap-4">
            ...
            history.map((h, i) => {
              const petInfo = PETS.find((p) => p.id === h.petId) || {
                name: 'Huge Inconnu',
                emoji: '🏆',
                image: null,
              };
              ...
              return (
                <div
                  key={h.id}
                  className="bg-[#111821] border border-slate-800/40 rounded-3xl p-5 flex items-center gap-4 shadow-lg hover:border-slate-700 transition-colors"
                >
                  {/* Avatar du Pet (Case dédiée) */}
                  <div className="w-20 h-20 shrink-0 bg-gradient-to-br from-slate-800 to-[#0a0f14] rounded-2xl flex items-center justify-center p-2 border border-slate-700/50 shadow-inner">
                    {petInfo.image ? (
                      <img
                        src={petInfo.image}
                        alt={petInfo.name}
                        className="max-w-full max-h-full object-contain drop-shadow-[0_5px_10px_rgba(0,0,0,0.5)]"
                        onError={(e) => {
                    ...
                    <span
                      ...
                      style={{ display: petInfo.image ? 'none' : 'block' }}
                    >
                      {petInfo.emoji}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col justify-center gap-3">
                    ...
                    <div className="flex justify-between items-start">
                      <span className="text-orange-500 font-black uppercase tracking-wider text-base drop-shadow-sm leading-none pt-1">
                        {petInfo.name}
                      </span>
                      <span className="text-[10px] text-slate-500 font-bold tabular-nums text-right leading-tight">
                        {h.date}
                      </span>
                    </div>

                    {/* Statistiques : Poids + Chance */}
                    <div className="flex justify-between items-end">
                      <div className="bg-emerald-500/10 text-emerald-400 px-4 py-2 rounded-xl border border-emerald-500/30 font-black flex flex-col justify-center shadow-sm min-w-[70px]">
                        <span className="text-[9px] opacity-70 uppercase tracking-widest mb-1">
                          Poids
                        </span>
                      ...
                      <div className="text-right">
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest leading-none mb-1.5">
                          Chance
                        </p>
        ...
        <div className="fixed inset-0 bg-black/98 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
          <div className="bg-[#111821] border border-slate-800 rounded-[3.5rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="bg-orange-500 p-6 flex justify-between items-center shrink-0">
              <h3 className="text-xl font-black text-[#060a0e] uppercase tracking-tighter italic">
                MAGNIFIQUE !
              </h3>
              <button
                onClick={() => {
                ...
                className="bg-black/10 p-2 rounded-full text-[#060a0e] transition-colors"
              >
                <X />
              </button>
            </div>

            <div className="p-8 space-y-6 text-center overflow-y-auto custom-scrollbar">
              <div className="space-y-1">
                ...
                <p className="text-5xl font-black text-white tracking-tighter tabular-nums">
                  {currentStreak}{' '}
                  <span className="text-orange-500 text-lg">œufs</span>
                </p>
              </div>

              {/* Sélection du Pet avec Barre de recherche */}
              <div className="text-left space-y-3">
                <div className="flex justify-between items-end ml-2 mb-1">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                    Quel Huge as-tu obtenu ?
                  </label>
                  <span className="text-[9px] text-slate-600 font-bold">
                    {filteredPets.length} trouvé(s)
                  </span>
                </div>
                      ...
                      <button
                        ...
                        className={`p-3 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-2 ${
                          selectedPet.id === pet.id
                            ...
                            : 'border-slate-800 bg-[#0a0f14] hover:border-slate-700 hover:bg-slate-900/50'
                        }`}
                      >
                        <div className="w-12 h-12 flex items-center justify-center">
                          ...
                          <span
                            ...
                            style={{ display: pet.image ? 'none' : 'block' }}
                          >
                            {pet.emoji}
                          </span>
                        </div>
                        <span className="text-[9px] font-black uppercase text-center text-slate-300 leading-tight">
                          {pet.name.replace('Huge ', '')}
                        </span>
                      </button>
                    ))
                  ) : (
                    <div className="col-span-3 text-center py-6 text-slate-600 text-xs font-bold uppercase tracking-widest">
                      Aucun résultat
                    </div>
                  )}
                </div>
              ...
              <div className="text-left space-y-3 pt-2 border-t border-slate-800/50">
                ...
                <input
                  ...
                  value={hugeWeight}
                  onChange={(e) => setHugeWeight(e.target.value)}
              ...
              <button
                onClick={handleRegisterHuge}
                className="w-full bg-orange-500 text-[#060a0e] font-black py-5 rounded-[2rem] text-xl shadow-xl hover:bg-orange-400 active:scale-95 transition-all uppercase tracking-[0.2em] border-b-4 border-orange-800 shrink-0"
              >
                ENREGISTRER
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#111821] border border-slate-800 rounded-[3rem] p-12 w-full max-w-xs shadow-2xl">
            ...
            <p className="text-slate-500 text-sm mb-10 font-medium">
              Tes statistiques et ton historique cloud seront perdus à jamais.
            </p>
            <div className="flex flex-col gap-4">
              <button
                onClick={() => setIsResetModalOpen(false)}
                className="bg-slate-800 py-5 rounded-2xl font-black uppercase tracking-widest text-xs border border-slate-700"
              >
                ANNULER
              </button>
              <button
                onClick={handleResetAll}
                className="bg-red-600 py-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-red-900/30 text-white"
              >
                OUI, TOUT EFFACER
              </button>
            </div>
